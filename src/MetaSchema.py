...
from src.MSchema_utils import examples_to_str, read_json, write_json
...
class MetaSchema:
    ...
    def add_description(self, description_dict):
        # Assume self.tables is already set up
        for table in self.tables:
            if(table in description_dict):
                table_des = description_dict[table]
                for col in self.tables[table]["fields"]:
                    if(col in table_des):
                        des = table_des[col]
                        original_comment = self.tables[table]["fields"][col]["comment"]
                        if(des and not original_comment):
                            self.tables[table]["fields"][col]["comment"] = des
    ...
    def process_schema(self, schema):
        ...
        # Step 1: Add tables to MSchema
        for i, table_name in enumerate(schema['table_names_original']):
            table_name_clean = schema['table_names'][i]
            self.add_table(table_name_clean)

        # Step 2: Add columns to the tables
        for i, column_info in enumerate(schema['column_names_original']):
            table_idx, column_name = column_info
            table_name = schema['table_names'][table_idx]
            column_type = schema['column_types'][i]

            # We assume that primary keys are specified by their index in primary_keys list
            primary_key = i in schema['primary_keys']
        ...
        # Step 3: Add foreign key relationships to MSchema
        for foreign_key in schema['foreign_keys']:
            # Foreign key format: [column_index, referenced_table_index]
            column_idx, ref_column_idx = foreign_key
            column_info = schema['column_names_original'][column_idx]
            ref_column_info = schema['column_names_original'][ref_column_idx]

            table_name = schema['table_names'][column_info[0]]
            ref_table_name = schema['table_names'][ref_column_info[0]]
            field_name = column_info[1]
            ref_field_name = ref_column_info[1]

            # Add the foreign key constraint
            self.add_foreign_key(
                table_name=table_name,
                field_name=field_name,
                ref_schema=None,  # Assuming all foreign keys are within the same schema
